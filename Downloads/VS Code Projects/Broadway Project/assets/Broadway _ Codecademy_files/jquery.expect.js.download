/*
  MIT License.
  Copyright (c) 2012 Amjad Masad <amjad@codecademy.com> Codecademy LLC
*/
...
(function (global, undefined) {
        ...
        this.assert(
          ...
          msg ||
            'expected ' +
              ...
              len,
          msg ||
            'expected ' + inspect(this.obj) + ' to not have a length of ' + len
        );
        return this;
      };
  ...
  Assertion.prototype.greaterThan = Assertion.prototype.above = function (
    n,
    msg
  ) {
    this.assert(
      this.obj.length > n,
      msg ||
        'expected ' + inspect(this.obj) + ' to have a length greater than ' + n,
      msg ||
        'expected ' + inspect(this.obj) + ' to have a length less than ' + n
    );
    return this;
  };
   ...
   * @api public
   */
  Assertion.prototype.eql = Assertion.prototype.equal = function ($el, msg) {
  ...
  Assertion.prototype.attr = function (prop, val, msg) {
    ...
    if (undefined === val) {
      this.assert(
        undefined !== got,
        msg ||
          'expected ' + inspect(this.obj) + ' to have an attribute ' + prop,
        msg ||
          'expected ' + inspect(this.obj) + ' not to have attribute ' + prop
      );
    } else {
      this.assert(
        got === val,
  ...
  function normalizeColor(color) {
    ...
    function r(x) {
      return x.toUpperCase() + x.toUpperCase();
    }

    if (color.match(/^#/)) {
      if (color.length === 4) {
        return (
          '#' + r(color.charAt(1)) + r(color.charAt(2)) + r(color.charAt(3))
        );
      }
  ...
  function compareQuad($el, prop, val) {
    ...
    var passing = true,
      got = $.map(['top', 'right', 'bottom', 'left'], function (dir, i) {
        ...
        if (isColor) {
          if (normalizeColor(value) !== (got = normalizeColor(got)))
            passing = false;
        } else {
  ...
  function borderQuad($el, prop, val) {
    ...
    var passing = true,
      got = $.map(['width', 'style', 'color'], function (style, i) {
        ...
        if (style === 'color') {
          if ((got = normalizeColor(got)) !== normalizeColor(val[i]))
            passing = false;
        } else {
  ...
  Assertion.prototype.css = function (prop, val, msg) {
    ...
    function check(obj) {
      switch (prop) {
        ...
        case 'color':
          passing =
            (got = normalizeColor(obj.css(prop))) === normalizeColor(val);
          break;
        ...
        case 'border-radius':
          got = [];
          passing =
            (got[0] = obj.css('border-top-left-radius')) === val &&
      ...
      that.assert(passing, template(got), template());
    }
      ...
      this.assert(
        ...
        msg ||
          'expected ' +
            ...
            text.length,
        msg ||
          'expected ' + inspect(this.obj) + ' text to not be of length ' + val
      );
    } else if (val instanceof RegExp) {
      this.assert(
        val.test(text),
        msg ||
          'expected ' + inspect(this.obj) + ' text to match ' + String(val),
        msg ||
          'expected ' + inspect(this.obj) + ' text not to match ' + String(val)
      );
    } else if (null == val) {
      this.assert(
        !!text.length,
      ...
      this.assert(
        ...
        msg ||
          'expected ' +
            ...
            text,
        msg ||
          'expected ' + inspect(this.obj) + ' text to not be equal to ' + val
      );
    }
    ...
    if (strict) {
      ...
      passing =
        this.obj
          ...
          .replace(re, '')
          .toLowerCase()
          .indexOf(text.replace(re, '').toLowerCase()) > -1;
    }
  ...
  $.each(
    ...
    function (_, fn) {
      Assertion.prototype[fn] = function (val, msg) {
        ...
        if (
          'string' === typeof val &&
          (ops[$.trim(val).slice(0, 2)] || ops[$.trim(val).charAt(0)])
        ) {
          val = $.trim(val);
   ...
   * @api public
   */
  Assertion.prototype.value = Assertion.prototype.val = function (val, msg) {
    var got;
    this.assert(
    ...
    function (_, fn) {
      Assertion.prototype[fn] = function (val, msg) {
        ...
        this.assert(
          ...
          msg ||
            'expected ' + inspect(this.obj) + ' not to have ' + fn + ' ' + val
        );
  ...
  Assertion.prototype.any = function (assertionFn, msg) {
    if (!(assertionFn instanceof Function)) {
      throw TypeError(
        'The any assertion must be passed an assertion ' +
  ...
  Assertion.prototype['class'] = function (className, msg) {
    ...
    this.obj.each(function (_, el) {
      that.assert(
        ...
        msg ||
          'expected ' + inspect(that.obj) + ' not to have class ' + className
      );
    });
